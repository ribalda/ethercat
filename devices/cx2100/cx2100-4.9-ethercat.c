        ...
        )
{
  ...
  if (len < ETH_ZLEN) memset(&txFrame->data, 0, ETH_ZLEN);
        ...
        )
{
  ...
  {
    // invalid information
    if      (baseInfoBlock->fnType != 0x0001)        MSG_ERR("Invalid base information type\n");
    else if (baseInfoBlock->_info.id1 != 0x000088a4) MSG_ERR("Invalid base information id 1\n");
    else if (baseInfoBlock->_info.id1 != 0x54414343) MSG_ERR("Invalid base information id 2\n");
  ...
  *ecMasterInfoBlock = baseInfoBlock+1;
  for (i = 0; i < baseInfoBlock->_info.fnBlockCount-1; i++)
  {
    ...
    if ((*ecMasterInfoBlock)->fnType == 0x14)
    {
      MSG_INFO("Found EtherCAT Master (0x14), Revision: %04x, txDMA: %02x, rxDMA: %02x\n", 
...
{
  MSG_INFO("EtherCAT-capable Beckhoff CX2100 EtherCAT Network Driver\n");
